...
struct TemporalSide<S: StateStore> {
    ...
    cache: ManagedLruCache<OwnedRow, Vec<OwnedRow>, DefaultHasher, SharedStatsAlloc<Global>>,
}
    ...
    fn update(&mut self, payload: Vec<StreamChunk>, join_keys: &[usize]) {
        payload.iter().flat_map(|c| c.rows()).for_each(|(op, row)| {
            ...
            if let Some(mut value) = self.cache.get_mut(&key) {
                match op {
                    Op::Insert | Op::UpdateInsert => value.push(row.into_owned_row()),
                    Op::Delete | Op::UpdateDelete => {
                        if let Some(pos) = value.iter().position(|r| Row::eq(r, row)) {
                            value.remove(pos);
                        }
                    }
                };
