...
use risingwave_common::types::{DataType, Date, Datum, Decimal, ScalarImpl, Time, F64};
...
fn do_parse_simd_json_value(
    ...
    let v = match dtype {
        ...
        DataType::Decimal => Decimal::try_from(F64::from(simd_json_ensure_float!(v, Decimal)))
